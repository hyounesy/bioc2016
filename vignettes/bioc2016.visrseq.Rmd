...
We will now make something more interesting. A kmeans clustering app. Like the previous example, we will start with a R script for kmeans clustering.
...
![](images/simple_kmeans.png)

here is the complete source code for ```simple_kmeans.R``` app:
```{r eval=FALSE}
# start parameter definition
visr.app.start("Simple Kmeans", debugdata = iris)
visr.category("clustering parameters")
